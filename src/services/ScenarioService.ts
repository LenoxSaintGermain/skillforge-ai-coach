...
export class ScenarioService {
  ...
  /**
   * Generates a new scenario using ADK 2.0 Multi-Agent Pipeline
   */
  public async generateScenario(
    ...
    try {
      ...
      const subject = description ? `${goalsText} - ${description}` : goalsText;
      
      const { supabase } = await import('@/integrations/supabase/client');
      const { data, error } = await supabase.functions.invoke('adk-course-creator', {
        body: { 
          subject,
          level: userProfile.ai_knowledge_level,
          role: userProfile.role
        }
      });

      if (error) {
        console.error('Error calling ADK Course Creator API:', error);
        throw new Error('Failed to generate scenario with AI');
      }

      const scenario: Scenario = {
        id: `adk-generated-${Date.now()}`,
        title: data.title || `Learning Scenario for ${userProfile.role}`,
        context: data.description || "Custom interactive scenario generated by ADK Team.",
        challenge: "Follow the interactive guide below.",
        tasks: [],
        resources: [],
        evaluationCriteria: [],
        skillsAddressed: learningGoals.map(g => g.skill_area),
        difficultyLevel: userProfile.ai_knowledge_level || 'Intermediate',
        estimatedTime: '90 minutes',
        completionStats: {
          percentComplete: 0,
          timeSpent: '0 minutes',
          skillProgress: learningGoals.map(g => ({
            skillName: g.skill_area,
            progress: 0
          })),
          coachInteractions: 0
        },
        a2uiPayload: data.a2uiPayload
      };
      
      return scenario;
      
    } catch (error) {
      console.error('Error generating ADK scenario:', error);
...
CRITICAL: You are an expert curriculum designer. Generate the entire lesson layout using A2UI JSON payloads.
Instead of a rigid JSON array, you MUST output a single valid JSON object representing the root of an A2UI component tree.
Do NOT wrap the JSON in markdown blocks like \`\`\`json. Just output the raw JSON object.

Use the following components to build an engaging, interactive lesson:
- Card, CardHeader, CardTitle, CardContent (for sections and explanations)
- Accordion, AccordionItem, AccordionTrigger, AccordionContent (for step-by-step tasks)
- Tabs, TabsList, TabsTrigger, TabsContent (for comparing approaches or prompts)
- Alert, AlertTitle, AlertDescription (for tips and warnings)
- Badge (for difficulty, time estimates, or skills addressed)

The scenario should include:
1. Title, Context, and Challenge Statement.
2. Specific hands-on tasks using Gemini or AI Studio.
3. Tips for evaluating output.
  ...
  "children": [
    {
      "type": "Card",
      "children": [
        { "type": "CardHeader", "children": [{ "type": "CardTitle", "children": "Scenario Title" }] },
        { "type": "CardContent", "children": "Scenario Context..." }
      ]
    },
    {
      "type": "Accordion",
  ...
  private parseScenarioResponse(
    ...
    try {
      ...
      const scenario: Scenario = {
        ...
        title: parsedData.title || `Learning Scenario for ${userProfile.role}`,
        context: parsedData.context || "Custom interactive scenario.",
        challenge: parsedData.challenge || "Follow the interactive guide below.",
        tasks: parsedData.tasks ? parsedData.tasks.map((task: any, index: number) => ({
        ...
        skillsAddressed: parsedData.skillsAddressed || learningGoals.map(g => g.skill_area),
        difficultyLevel: parsedData.difficultyLevel || userProfile.ai_knowledge_level || 'Intermediate',
        estimatedTime: parsedData.estimatedTime || '90 minutes',
        completionStats: {
          percentComplete: 0,
          timeSpent: '0 minutes',
          skillProgress: (parsedData.skillsAddressed || learningGoals.map(g => g.skill_area)).map((skill: string) => ({
            skillName: skill,
            progress: 0
          })),
          coachInteractions: 0
        },
        a2uiPayload: parsedData
      };
