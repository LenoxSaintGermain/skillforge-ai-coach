-- Phase 1: Multi-Subject Learning Framework - Database Foundation
-- This migration creates the foundation for supporting multiple learning subjects
-- while maintaining full backward compatibility with existing Gemini Training

-- Step 1: Create enum for subject status
CREATE TYPE public.subject_status AS ENUM ('draft', 'active', 'archived');

-- Step 2: Create learning_subjects table (master configuration)
...
-- Step 3: Create user_subject_enrollments table
CREATE TABLE public.user_subject_enrollments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Step 4: Modify content_cache to support multiple subjects (additive, non-breaking)
ALTER TABLE public.content_cache 
ADD COLUMN subject_id UUID REFERENCES learning_subjects(id);
...
ALTER TABLE public.user_subject_enrollments ENABLE ROW LEVEL SECURITY;

-- Step 6: RLS Policies for learning_subjects
...
-- Step 7: RLS Policies for user_subject_enrollments
CREATE POLICY "Users can view their enrollments"
ON public.user_subject_enrollments
FOR SELECT
TO authenticated
...
-- Step 8: Trigger for updated_at timestamp
CREATE TRIGGER update_learning_subjects_updated_at
BEFORE UPDATE ON public.learning_subjects
FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at_column();

-- Step 9: Seed the Gemini Training subject with full syllabus data
INSERT INTO public.learning_subjects (
  ...
  '["Gemini Training", "Prompt Engineering", "AI Implementation", "Model Training", "API Integration"]'::jsonb,
  true,
  'active'
);

-- Step 10: Auto-enroll all existing users into Gemini Training
-- This ensures complete backward compatibility
INSERT INTO public.user_subject_enrollments (user_id, subject_id, is_primary, last_accessed)
SELECT 
  u.id as user_id,
  ls.id as subject_id,
  true as is_primary,
...
FROM auth.users u
CROSS JOIN public.learning_subjects ls
WHERE ls.subject_key = 'gemini-training'
ON CONFLICT (user_id, subject_id) DO NOTHING;

-- Step 11: Update enrollment count for Gemini subject
UPDATE public.learning_subjects
SET total_enrollments = (
  SELECT COUNT(*) 
  FROM public.user_subject_enrollments 
  WHERE subject_id = learning_subjects.id
)
WHERE subject_key = 'gemini-training';